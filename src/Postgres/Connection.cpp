...
void Connection::startupMessage(std::string const& username, std::string const& database, std::string const& password, Options const& options)
{
    ...
    while (startupMessage)
    {
        ...
        if (response->isError())
        {
            throw std::runtime_error(errorMsg("ThorsAnvil::DB::Postgres::Connection::startupMessage: ", "Got an Error: ", *response));
        }

        std::unique_ptr<Authentication> auth(dynamic_cast<Authentication*>(response.get()));
        if (!auth)
        {
            throw std::domain_error(bugReport("ThorsAnvil::DB::Postgres::Connection::startupMessage: ", "Unexpected Response: ", *response));
        }
        response.release();
...
std::unique_ptr<Message> Connection::getMessage()
{
    ...
    switch (message)
    {
        ...
        default:
            throw std::domain_error(bugReport("ThorsAnvil::DB::Postgres::Connection::getMessage: ", "Unknown Message Type: ", message));
    }
}
...
std::unique_ptr<Message> Connection::getAuthenticationMessage()
{
    ...
    switch (authMessageType)
    {
        ...
        default:
            throw std::domain_error(bugReport("ThorsAnvil::DB::Postgres::Connection::getAuthenticationMessage: ", "Invalid Authentication Reply: ", authMessageType));
    }
}
