...
{
    p.writeLengthEncodedString(val);
    return true;
}

template<>
inline bool writeParameterType<std::string>(ConectWriter& p, std::string const&)
{
    p.writeFixedLengthInteger<1>(MYSQL_TYPE_VAR_STRING);
    p.writeFixedLengthInteger<1>(0);
    return true;
}

template<>
inline bool writeParameterValue<int>(ConectWriter& p, int const& val)
{
    p.writeFixedLengthInteger<4>(val);
    return true;
}

template<>
inline bool writeParameterType<int>(ConectWriter& p, int const&)
{
    p.writeFixedLengthInteger<1>(MYSQL_TYPE_LONG);
    p.writeFixedLengthInteger<1>(0);
    return true;
}
