...
SSLMethod::SSLMethod(SSLMethodType type)
    : method(type == SSLMethodType::Client ? SSLv23_client_method() : SSLv23_server_method())
{
    SSLUtil::getInstance();
}
...
void SSLObj::connect()
{
    ...
    if (ret != 1)
    {
        throw std::runtime_error(errorMsg("ThorsAnvil::SQL::SSLObj::SSLObj: SSL_connect() failed: ", SSLUtil::sslError(ssl, ret), " - ", SSLUtil::errorMessage()));
    }
}
