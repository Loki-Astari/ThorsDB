...
#ifndef THORSANVIL_DB_MONGO_CMD_ADM_H
...
CmdAdm_Query<Document>::CmdAdm_Query(Op_QueryOptions const& options, Args&&... args)
    : Op_Query<Document>("admin.$cmd", options, collection, std::forward<Args>(args)...)
{}
...
CmdAdm_Query<Document>::CmdAdm_Query(Op_QueryOptions const& options, typename Doc::Options const& docOpt, Args&&... args)
    : Op_Query<Document>("admin.$cmd", options, docOpt, std::move(collection), std::forward<Args>(args)...)
{}
